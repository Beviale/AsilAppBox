 ...
 * Use the {@link WeightTestFooterRun#newInstance} factory method to
 * create an instance of this fragment.
 */
public class WeightTestFooterRun extends BaseFooterFragment {
    private Button RUN_TEST_BUTTON;
    private final int TEST_DURATION_IN_MS = 5000;
    private Integer resultData;

    public WeightTestFooterRun() {
     ...
     */
    public static WeightTestFooterRun newInstance() {
        WeightTestFooterRun fragment = new WeightTestFooterRun();
    ...
    public void onStart() {
        ...
        RUN_TEST_BUTTON.setOnClickListener(v -> {
            ...
            WeightFragmentMain.TEST_PROGRESS_BAR.setVisibility(View.VISIBLE);
            WeightFragmentMain.TEST_PROGRESS_BAR.setProgress(0);

            // Creo un Thread che mi arresta automaticamente dopo 5 secondi il listener (raccolgo dati per soli 5 secondi)
            new Handler().postDelayed(() -> {
                WeightFragmentMain.TEST_PROGRESS_BAR.setVisibility(View.INVISIBLE);
                calculateTestResult();
                ...
                changeScreen(TipoMisurazioneEnum.PESO, String.valueOf(this.resultData));
            }, TEST_DURATION_IN_MS);

            // Imposto l'animazione per la progress bar
            ValueAnimator animator = ValueAnimator.ofInt(0, WeightFragmentMain.TEST_PROGRESS_BAR.getMax());
            animator.setDuration(TEST_DURATION_IN_MS);
            animator.addUpdateListener(animation -> WeightFragmentMain.TEST_PROGRESS_BAR.setProgress((Integer)animation.getAnimatedValue()));
            animator.start();
    ...
    protected void calculateTestResult() {
        resultData = getRandomNumberInRange(40, 151);
    }
